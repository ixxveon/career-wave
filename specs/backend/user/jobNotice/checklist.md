...
- [x] `JobNotice` 엔티티 필드가 `job_notices` ERD 컬럼명, nullable 조건, PK/UNIQUE/CHECK 제약과 일치한다.
- [x] `Bookmark` 엔티티 필드가 `bookmarks` ERD 컬럼명, PK/UNIQUE/FK 제약과 일치한다.
- [x] `JobNoticeStatus`, `JobType`, `CompanySize`, `CareerLevel` Enum 값이 DB CHECK 제약조건과 일치한다.
