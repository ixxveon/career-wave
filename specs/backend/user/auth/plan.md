...
| verification 저장 | DB `member_verifications` | `db/init.sql` ERD 기준, 가입 전 인증도 회원 FK 없이 target 기준 관리 |
| verificationToken | `member_verifications.verification_token` | 프론트 boolean 신뢰 금지, 서버 권위 검증 |
...
| 기업 정보 검증 | 외부 API adapter 사용 | 사업자등록번호/대표자명/기업정보는 프론트 입력만 신뢰하지 않음 |
...
- 인증번호/verification session은 DB `member_verifications` 기준으로 설계한다.
...
- `MemberVerificationRepository`
  ...
  - `existsRecentByTargetAndPurpose`
  - `countRecentSentByTargetAndPurpose`
- `HrManagerRepository`
  - `findByMemberId`
...
- `UserVerificationService`
  - 인증번호 발송
  - 인증번호 확인
  - verificationToken 발급
  - purpose/target/channel 검증
  - rate limit 처리
- `UserRecoveryService`
  - 아이디 찾기
  - loginId 마스킹
