...
- [ ] `WebSocketConfig` 설정 클래스 작성
  - `WebSocketHandshakeInterceptor` + `ResumeWebSocketHandler` 등록
  - 엔드포인트: `/ws/user/resume/{documentId}/status`
- [ ] `WebSocketHandshakeInterceptor` 구현
  - `beforeHandshake()`: 쿼리 파라미터 `?token=` 추출 → JWT 검증 → `memberId` 세션 attributes 저장
  - `documentId` 소유권 확인(`DocumentRepository`) — 불일치 시 핸드셰이크 거부 (HTTP 1008)
  - 검증 실패(토큰 만료·미소유) 시 `false` 반환으로 연결 거부
- [ ] `ResumeWebSocketHandler` 구현 (`TextWebSocketHandler` 상속)
  - `afterConnectionEstablished()`: 현재 `document.status` 조회 후 Snapshot 1회 전송 (재연결 대응)
  - 세션 등록: `ConcurrentHashMap<UUID, WebSocketSession>`에 `documentId` 키로 저장
  - `afterConnectionClosed()`: 세션 Map에서 제거 + 진행 중 Grace Period 타이머 취소
- [ ] Webhook 수신 후 세션 Map에서 `documentId`로 `WebSocketSession` 조회 → `sendMessage()` 연동
...
- [ ] 엔드포인트 `/ws/user/resume/{documentId}/status` 연결·메시지 수신 동작 확인
