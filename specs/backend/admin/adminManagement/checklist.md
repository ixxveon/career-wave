...
> tasks.md가 "무엇을 만들지"를 정의한다면, checklist.md는 "제대로 만들었는지"를 검증한다.
> 구현 완료 후 PR을 올리기 전에 작성자가 직접 점검한다.
...
- [x] 비즈니스 로직은 Controller나 Repository가 아니라 Service Layer에 위치한다.
- [x] Service가 인터페이스와 `impl` 구현체로 분리되어 있다.
- [x] 관리자 이메일 중복 검증이 Service Layer에서 수행된다.
- [x] IP ACL 범위 중복 검증이 Service Layer에서 수행된다.
- [x] 관리자 상태 중복 변경(`ACTIVE`, `LOCKED`)은 Service Layer에서 차단된다.
- [x] IP ACL 활성 상태 중복 변경(`isEnabled`)은 Service Layer에서 차단된다.
...
- [x] 모든 성공 응답은 `ApiResponse<T>` 래퍼 형식을 사용한다.
- [x] 모든 성공 응답이 `ApiResponse<T>` 규격(`success`, `statusCode`, `message`, `data`)과 일치한다.
- [x] 목록 조회 응답은 `content`, `page`, `size`, `totalElements`, `totalPages` 구조를 사용한다.
- [x] 관리자 계정/ACL 생성, 수정, 삭제 API에는 `page`, `size` Query Parameter가 노출되지 않는다.

## Phase 5 - Documentation

- [x] `AdminManagementDocs` 인터페이스가 작성되어 있다.
...
- [x] `api-schema.md`의 Query Parameter와 ERD 컬럼 매핑이 실제 구현과 일치한다.
- [x] `api-schema.md`의 Enum 값이 실제 구현 Enum 및 DB CHECK 제약과 일치한다.
- [x] 도메인 ErrorCode 문서가 실제 예외 처리와 일치한다.
...
- [x] 외부 보안 장비나 별도 ACL 시스템과 직접 연동하는 코드가 본 도메인 구현에 포함되지 않는다.
