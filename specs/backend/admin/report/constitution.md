...
| 결정 | 내용 | 근거 |
|------|------|------|
| 콘텐츠 블라인드 처리 방식 | `admin/report/` 내에 `ReportBoardRepository`, `ReportCommentRepository`를 별도 정의하여 같은 `boards`/`comments` 테이블에 접근한다. `user/` 패키지 클래스를 import하지 않는다. | CONVENTION.md — admin과 user는 패키지 클래스를 직접 import하지 않는다. 같은 DB 테이블을 admin 전용 Repository로 따로 정의하는 것은 허용 |
...
- 프론트엔드 계약 원본: `specs/frontend/admin/report/api-schema.md`
- 제공 엔드포인트:
