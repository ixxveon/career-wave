...
- [ ] 목록 조회 쿼리가 `logType`, `severity`, `keyword`, `from`, `to`, `page`, `size` 조건을 반영하는가
- [ ] Query Parameter와 ERD 컬럼 매핑이 문서 기준과 일치하는가
...
- [ ] `from`, `to`, `page`, `size`의 유효하지 않은 요청이 공통 요청 검증 실패 `400`으로 처리되는가
- [ ] `from > to` 요청이 공통 요청 검증 실패 `400`으로 처리되는가
- [ ] `keyword` 공백-only 입력이 필터 미적용으로 처리되는가
- [ ] `keyword` 100자 초과 요청이 공통 요청 검증 실패 `400`으로 처리되는가
- [ ] 허용되지 않은 `logType` 요청이 `INVALID_AUDIT_LOG_TYPE`으로 처리되는가
- [ ] 허용되지 않은 `severity` 요청이 `INVALID_AUDIT_LOG_SEVERITY`로 처리되는가
- [ ] 존재하지 않는 `logId` 요청이 `AUDIT_LOG_NOT_FOUND`로 처리되는가
- [ ] 공통 `ErrorCode`와 도메인 `AuditLogErrorCode`가 분리되어 사용되는가
- [ ] 조회 조건 검증, 페이지 번호 변환, 권한 분기가 Controller나 Repository가 아니라 Service 계층에 위치하는가
...
- [ ] 외부 API 기준 `page` 1-based 입력이 내부 Pageable 변환 시 `page - 1`로 정확히 처리되는가
- [ ] 감사 로그 조회 도메인에 생성, 수정, 삭제 API가 추가되지 않았는가

## Phase 4 - Documentation

- [ ] `AuditLogDocs` 인터페이스가 작성되어 있고 Swagger 어노테이션이 Controller에서 분리되어 있는가
- [ ] Controller에 Swagger 어노테이션이 직접 작성되어 있지 않은가
- [ ] `api-schema.md`의 요청 조건, 응답 필드, 권한 정책, ErrorCode 계약이 실제 구현과 일치하는가
...
- [ ] 비즈니스 로직이 Service Layer에만 존재하고 Controller는 요청 바인딩과 응답 반환만 담당하는가
- [ ] Repository가 인가 정책, 응답 포맷, ErrorCode 매핑 책임까지 가지지 않는가
- [ ] Entity를 API 응답으로 직접 노출하지 않는가

## 머지 전 최종 확인

- [ ] 권한(Role) 정책이 문서 기준(`MASTER`, `BACKEND`만 허용)과 실제 구현에서 일치하는가
- [ ] Audit Log 기록 여부 측면에서 본 도메인이 조회 전용 범위를 유지하고, 조회 요청 때문에 새로운 감사 로그를 생성하지 않는가
