...
- [x] `from`, `to`, `page`, `size`가 유효하지 않은 요청을 공통 요청 검증 실패 `400`으로 처리하는가
- [x] `keyword` 100자 초과 요청을 공통 요청 검증 실패 `400`으로 처리하는가
- [x] `GET /api/v1/admin/audit-logs/summary`가 구현되어 있고 관리자 인증(`ROLE_ADMIN`) + `MASTER`, `BACKEND` 세부 역할로만 접근 가능한가
- [x] `GET /api/v1/admin/audit-logs`가 구현되어 있고 감사 로그 목록을 페이지네이션으로 반환하는가
- [x] `GET /api/v1/admin/audit-logs/{logId}`가 구현되어 있고 관리자 인증(`ROLE_ADMIN`) + `MASTER`, `BACKEND` 세부 역할로만 접근 가능한가
...
- [x] 외부 API 기준 `page` 1-based 입력이 내부 Pageable 변환에서만 `page - 1`로 처리되는가
...
- [x] 권한(Role) 정책이 문서 기준(관리자 인증(`ROLE_ADMIN`) + `MASTER`, `BACKEND` 세부 역할 허용)과 실제 구현에서 일치하는가
- [ ] Audit Log 기록 여부 측면에서 본 도메인이 조회 전용 범위를 유지하고, 조회 요청 자체로는 감사 로그를 생성하지 않는가
- [ ] `constitution.md`의 분리 규칙(조회 전용 유지, 권한 제한, enum 제약, 1-based 페이지네이션 등)이 구현과 테스트에 반영되었는가
