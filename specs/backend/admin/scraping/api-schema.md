...
| `actionType` | `RUN \| RETRY \| TEST` | Y | 없음 | 액션 유형 |
| `accepted` | `boolean` | Y | 없음 | 접수 여부 |
| `pipelineStatus` | `IDLE \| RUNNING \| SUCCESS \| FAILED` | Y | FastAPI 응답 기준 `scraping_pipelines.pipeline_status` | 접수 직후 상태 |
...
| Field | Type | Required | Mapping | Description |
|---|---|---|---|---|
| `scrapingLogId` | `number` | Y | FastAPI 응답 기준 `scraping_logs.scraping_log_id` | 로그 ID |
| `scrapingPipelineId` | `number` | N | FastAPI 응답 기준 `scraping_logs.scraping_pipeline_id` | 파이프라인 ID |
| `sourceName` | `string` | N | FastAPI 응답 기준 `scraping_pipelines.source_name` | 소스명 |
| `targetSite` | `string` | Y | FastAPI 응답 기준 `scraping_logs.target_site` | 대상 사이트 |
| `scrapingStatus` | `SUCCESS \| FAILED` | Y | FastAPI 응답 기준 `scraping_logs.scraping_status` | 실행 결과 상태 |
| `totalCount` | `number` | N | FastAPI 응답 기준 `scraping_logs.total_count` | 수집 건수 |
| `errorMessage` | `string` | N | FastAPI 응답 기준 `scraping_logs.error_message` | 오류 메시지 |
| `executedAt` | `string` | Y | FastAPI 응답 기준 `scraping_logs.executed_at` | 실행 시각 |
