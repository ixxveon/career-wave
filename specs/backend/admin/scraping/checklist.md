...
- [x] 단일 액션 요청 후 Audit Log 기록 로직이 존재한다.
- [x] 배치 액션 요청 후 Audit Log 기록 로직이 존재한다.
...
- [x] 배치 액션은 `actionType`과 `batch-run` 요청을 누락 없이 전달한다.
...
- [x] 외부 API는 `page`가 1-based로 동작하고 내부 변환에만 `page - 1`이 적용된다.

## Phase 6 - Security / Docs / Layering

- [ ] `scraping` 도메인 API 권한을 `MASTER`, `BACKEND`로만 제한한다.
