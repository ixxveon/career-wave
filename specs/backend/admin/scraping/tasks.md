...
- [x] `ScrapingPipelineRepository.java`를 엔티티 매핑 및 최소 조회 목적 기준으로 작성한다.
- [x] `sourceName` 기준 단일 파이프라인 조회 메서드를 정의한다.
...
- [x] 외부 1-based `page`를 검증하고 Spring 내부 Pageable 지점에서만 `page - 1` 변환 로직을 구현한다.
...
- [x] `ScrapingDocs.java` Swagger 인터페이스를 작성한다.
- [x] 관리자 목록/요약/상세/로그 조회 API 문서를 정리한다.
- [x] 단일 액션 및 배치 액션 API 문서를 정리한다.
- [x] FastAPI 내부 API 매핑과 ErrorCode 매핑이 백엔드 문서와 일치하도록 정리한다.
- [x] Audit Log 기록 범위를 문서와 구현 계획에 맞춘다.

## Phase 7 - Test

- [x] FastAPI 목록/요약/상세/로그 응답 매핑 테스트를 작성한다.
- [x] 단일 실행/재시도/테스트 액션 요청 테스트를 작성한다.
- [x] 배치 액션 요청 테스트를 작성한다.
- [x] `page - 1` 변환 테스트를 작성한다.
- [x] ErrorCode 변환 테스트를 작성한다.
- [x] 중복 실행 차단 테스트를 작성한다.
- [x] Audit Log 기록 테스트를 작성한다.
