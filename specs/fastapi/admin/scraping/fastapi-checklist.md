...
- [ ] `source + original_url` 중복 제거 기준이 저장 로직에도 동일하게 적용된다.
...
- [x] GET 내부 API Query Parameter 테스트가 존재한다.
- [x] Source Registry 테스트가 존재한다.
- [x] 실행/재시도/TEST/배치 액션 테스트가 존재한다.
- [x] TEST 저장 제외 테스트가 존재한다.
- [x] 중복 실행 차단 테스트가 존재한다.
- [x] 중복 제거 및 저장 테스트가 존재한다.
- [x] 상태 전이 테스트가 존재한다.
- [x] 실행 로그 기록 테스트가 존재한다.
