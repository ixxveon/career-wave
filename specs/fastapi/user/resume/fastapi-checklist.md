...
- [ ] **파일 스트리밍**: S3 다운로드 시 `response.read()` 한 번에 메모리에 올리지 않고, `.stream()` 또는 청크 단위로 읽어 파일 시스템에 쓰는가? (`response.read()` 한 줄이면 대용량 파일에서 OOM 발생)
- [ ] **리소스 정리**: 파싱 도중 예외가 발생하더라도 `finally` 블록에서 `os.remove()`가 반드시 실행되는가? (임시 파일 누적으로 디스크 고갈 방지)
- [ ] **Webhook 무결성**: Spring Boot가 Out-of-order Webhook(현재 DB 상태보다 이전 상태)을 무시하는 로직만으로 충분하며, 별도 idempotency-key 없이도 안정적으로 동작하는가?
