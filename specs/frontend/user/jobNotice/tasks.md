...
- [x] 검색 입력, 필터, 정렬, 스크랩 버튼, 원본 공고 버튼, 상세 닫기 버튼, 상단 이동 버튼에 접근 가능한 이름을 제공한다.
- [x] 상세 드로어는 `role="dialog"`와 `aria-modal="true"`를 제공한다.
- [x] 상세 드로어가 열린 동안 배경 스크롤을 막고 Escape 키로 닫을 수 있게 한다.
- [x] 상세 드로어 닫기 버튼, 오버레이 클릭, Escape 키 닫기가 모두 동작한다.
- [x] 상세 탭은 키보드로 이동하고 선택할 수 있어야 한다.
- [x] 1320px, 1080px, 720px 이하 레이아웃에서 카드 그리드와 필터 패널이 깨지지 않는지 확인한다.
- [x] 360px 모바일 화면에서 목록 카드, 필터, 상세 드로어, 하단 액션 버튼 텍스트가 겹치지 않는지 확인한다.
- [x] 빈 결과 상태에서 필터를 줄이거나 검색어를 변경할 수 있는 흐름을 유지한다.
- [x] 주요 상호작용을 수동 검증한다: 검색, 인기 검색어, 필터 선택/해제, 기간 선택, 정렬, 목록 스크랩, 상세 열기/닫기, 상세 탭 전환, 상세 스크랩, 원본 이동, 상단 이동.
- [x] `npm run build` 또는 프로젝트 표준 검증 명령으로 타입/빌드 오류를 확인한다.
- [x] 문서만 수정한 PR이 아니며 빌드를 수행했으므로 빌드 생략 사유는 PR 본문에 남기지 않는다.
