...
- [ ] `api-schema.md`의 `JobNoticeSummary`, `JobNoticeDetail`, `JobNoticeBookmarkResponse` 필드를 프론트엔드 타입과 비교한다.
- [ ] API 성공/실패 응답이 `ApiResponse<T>`의 `success`, `statusCode`, `message`, `data` 필드를 기준으로 문서화되어 있는지 확인한다.
- [ ] `JobNoticeTypes.ts`의 `exp`, `employment` 화면 필드와 API의 `experience`, `employmentType` 필드 매핑 규칙을 정한다.
- [ ] 상세 필드 `industry`, `responsibilities`, `requirements`, `preferredQualifications`, `process`, `workConditions`, `companyDescription` 타입을 추가하거나 변환 계층에서 보강한다.
- [ ] 필터 enum(`직무`, `경력`, `채용 유형`, `지역`, `기업 규모`)과 정렬 enum(`추천순`, `최신순`, `조회순`)을 상수로 고정한다.
- [ ] 목록과 상세가 같은 공고 `id`와 `bookmarked` 값을 참조하도록 상태 소유 지점을 정한다.
...
- [ ] 공고 카드는 클릭과 Enter/Space 키로 상세 드로어를 열 수 있게 한다.
- [ ] 공고 카드 선택 시 `JobNoticeDetail`에 선택 공고 요약 데이터와 현재 스크랩 상태를 전달한다.
- [ ] 상세 조회 호출은 `frontend/src/user/api` 하위 API 모듈에 둔다.
- [ ] API 연동 시 `getJobNoticeDetail(jobNoticeId)`를 TanStack Query 또는 도메인 hook으로 관리하는 방식을 우선 검토한다.
- [ ] 상세 드로어는 목록 요약 데이터로 즉시 열고, 상세 응답 도착 후 상세 필드를 보강한다.
- [ ] 상세 조회 상태를 `loading`, `success`, `empty`, `error`로 구분한다.
- [ ] 상세 조회 실패 시 목록을 비우지 않고 드로어 내부에 오류 메시지와 닫기 또는 재시도 액션을 표시한다.
- [ ] 상세 조회 결과가 현재 선택 공고 `id`와 다르면 화면에 반영하지 않는다.
- [ ] 상세 드로어는 핵심 정보 카드, 기술 스택, `공고 상세` 탭, `기업 정보` 탭을 표시한다.
- [ ] `공고 상세` 탭은 담당 업무, 자격 요건, 우대 사항, 전형 절차, 근무 조건, 마감 정보를 표시한다.
- [ ] `기업 정보` 탭은 회사명, 업종, 기업 규모, 기업 소개를 표시한다.
- [ ] 상세 필드가 비어 있으면 빈 리스트 대신 fallback 안내 또는 해당 섹션 숨김을 적용한다.
- [ ] 원본 공고 이동은 `originalUrl`을 우선 사용하고, 없으면 제목 기반 검색 URL을 fallback으로 사용한다.
- [ ] 원본 이동 URL을 구성할 수 없으면 원본 이동 버튼을 비활성화하고 안내 문구를 표시한다.
...
- [ ] 스크랩 토글 호출은 `frontend/src/user/api` 하위 API 모듈에 둔다.
- [ ] 목록 카드와 상세 드로어의 스크랩 버튼은 같은 `id` 기준으로 현재 상태를 표시한다.
- [ ] 스크랩 저장/해제 API 성공 시 목록 카드와 상세 드로어의 상태를 동시에 갱신한다.
- [ ] 스크랩 API 실패 시 버튼 상태를 이전 값으로 되돌리고 사용자에게 실패 안내를 제공한다.
- [ ] 비로그인 사용자가 스크랩을 시도하면 로그인 필요 안내 또는 `/auth/login` 이동을 제공한다.
- [ ] 스크랩 API 응답의 `scrapCount`가 필요한 화면이 있으면 같은 공고 상태에 반영한다.
