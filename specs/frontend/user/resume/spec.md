...
**Acceptance Scenarios**:
1. **Given** 이력 목록 진입 시, **Then** 본인의 분석 이력이 최신순으로 무한스크롤로 표시된다.
...
- **FR-004**: 이력 목록 최신순 무한스크롤 조회 (`useInfiniteQuery` + IntersectionObserver, 페이지당 5건).
- **FR-005**: 분석 완료 후 면접 도메인 연동(`?documentId=xxx`).
- **FR-006**: AI 분석 작업 취소(Abort) 및 `IDLE` 상태 복귀 기능 제공. (클라이언트 사이드 — WebSocket 구독 해제 후 상태 초기화. 별도 서버 취소 API 없음. **서버는 취소 후에도 분석을 계속 완료하여 DB에 저장하며**, 클라이언트는 해당 결과를 더 이상 구독하지 않는 것으로 처리한다. 추후 이력 목록에서 `COMPLETED` 상태로 재조회 가능.)

### Non-functional Requirements
- **NFR-001**: 분석 완료까지 30초 이내 처리(Timeout 시 안내).
- **NFR-002**: Lighthouse 성능 90/접근성 95점 이상.
- **NFR-003**: 본인 소유가 아닌 문서 접근 차단(IDOR 방어). 리포트 조회(`GET /feedback`) 및 WebSocket 구독 시, 요청한 `documentId`가 인증된 `memberId`와 매핑되는지 API 서버에서 검증한다. 불일치 시 서버가 `403` 반환 → 클라이언트는 이력 목록으로 리다이렉트.
...
- **파일 파싱 실패**: 암호화된 PDF 또는 텍스트 추출이 불가능한 이미지 기반 PDF 업로드 시 서버가 파싱 오류를 반환한다. 클라이언트는 `FAILED` 상태 수신 시 `errorMessage` 필드 값을 우선 노출하고, `errorMessage`가 null이면 기본 문구("분석 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.")를 노출한 뒤 `IDLE` 상태로 복귀한다.
