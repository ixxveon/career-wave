...
- **파일 파싱 실패**: 암호화된 PDF 또는 텍스트 추출이 불가능한 이미지 기반 PDF 업로드 시 서버가 파싱 오류를 반환한다. 클라이언트는 `FAILED` 상태 수신 시 "파일을 읽을 수 없습니다. PDF가 암호화되어 있거나 이미지로만 구성된 경우 분석이 어렵습니다." 안내 문구를 노출하고 `IDLE` 상태로 복귀한다.
