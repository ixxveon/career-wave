...
- [x] Career History 도메인의 핵심 기능 범위를 정리한다.
- [x] 취업 준비 기록 및 히스토리에서 관리할 데이터 항목을 정의한다.
- [x] 면접 연습 기록에 포함될 텍스트/비디오 모의면접 데이터 구조를 정의한다.
- [x] 누적 데이터 기반 AI 역량 평가에 필요한 점수 항목을 정의한다.
- [x] 맞춤형 로드맵 가이드에 필요한 약점 분석 및 추천 액션 플랜 구조를 정의한다.
- [x] Career History와 Document Analysis, Interview 도메인 간 연관 데이터를 확인한다.
...
- [x] 취업 준비 기록 목록 조회 API를 구현한다.
- [x] 취업 준비 기록 상세 조회 API를 구현한다.
- [x] 날짜별 취업 준비 기록 조회 기능을 구현한다.
- [x] 기업별 취업 준비 기록 조회 기능을 구현한다.
- [ ] 면접 연습 기록 조회 API를 구현한다.
- [x] 질문-답변 매칭 데이터 조회 기능을 구현한다.
- [x] AI 오답 하이라이팅 및 개선 피드백 조회 기능을 구현한다.
- [x] 누적 서류 점수 및 면접 점수 기반 역량 평가 조회 API를 구현한다.
- [x] 약점 분석 및 우선 학습 타겟 조회 기능을 구현한다.
- [x] 맞춤형 로드맵 조회 API를 구현한다.
...
- [x] API 응답 DTO와 프론트엔드 화면 데이터 구조가 일치하는지 확인한다.
- [x] 빈 데이터, 일부 누락 데이터, 존재하지 않는 상세 기록 접근 케이스를 테스트한다.
- [ ] 통합 테스트 작성한다.