...
* 비밀번호, 인증번호, 주민성 민감 데이터는 `localStorage`에 저장하지 않는다.
* access token은 메모리에 우선 보관하고, refresh token은 HttpOnly cookie를 기본으로 하되 필요 시 `sessionStorage` 기반 탭 세션 복원 전략을 사용할 수 있다.
