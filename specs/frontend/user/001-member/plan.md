...
- [ ] `member.ts` — API DTO, 회원 유형, 계정 상태, 기업 승인 상태, 인증 상태 타입 정의
- [ ] `api/member/` — 로그인/가입/인증/복구 API 함수 인터페이스 작성
- [ ] TanStack Query 또는 인증 훅 queryKey 컨벤션 정의 (`me`, `memberStatus`, `companyApprovalStatus`)
- [ ] 공통 에러 매핑 작성 (`401`, `403`, `409`, `423`, `429`)
- [ ] mock 응답과 실제 API 응답을 분리하는 adapter 설계

### Phase 2: 로그인 및 세션 진입
- [ ] 개인/기업 로그인 탭 UI 및 폼 검증 구현
- [ ] 로그인 요청 중복 제출 방지 및 실패 메시지 일반화
- [ ] 로그인 성공 후 회원 상태 조회 및 protected route 진입 제어
- [ ] 계정 정지/잠금/블랙리스트/기업 승인 대기 안내 UI 구현
- [ ] 토큰 만료 및 401 응답 공통 처리

### Phase 3: 회원가입 및 인증
- [ ] 개인회원 가입 폼 구현: 아이디, 이메일, 휴대폰, 비밀번호, 약관
- [ ] 기업회원 가입 폼 구현: 기업정보, 담당자 정보, 재직증명서 PDF
- [ ] 아이디 중복 확인, 인증번호 발송/확인/재전송 쿨다운 구현
- [ ] 비밀번호 정책 및 필수 약관 검증 구현
- [ ] PDF 확장자/MIME/size 검증 및 서버 검증 실패 UI 반영

### Phase 4: 계정 복구 플로우
- [ ] `/auth/find-account` 개인/기업 선택 카드 구현
- [ ] 개인/기업 아이디 찾기 인증 플로우 구현
- [ ] 개인/기업 비밀번호 재설정 인증 플로우 구현
- [ ] 마스킹된 아이디 결과와 보안 문구 정책 적용
- [ ] reset token 만료/재시도 상태 처리
