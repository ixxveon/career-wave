...
- [ ] 필터·검색 변경이 즉시 API 호출을 트리거하지 않는다.
- [ ] 검색 버튼 클릭 또는 Enter 입력 시에만 fetch가 호출된다.
- [ ] API 응답 처리 시 `res.data.success`를 먼저 확인한 후 `res.data.data`에 접근한다.
- [ ] 모든 HTTP 호출이 `supportApi.ts`를 통해서만 수행된다.
- [ ] 카테고리 값은 API 송수신 시 ERD 영문, UI 표시 시 한글 변환 상수를 사용한다.
- [ ] 미로그인 상태에서 문의 관련 기능 접근 시 `/auth/login`으로 리다이렉트된다.
- [ ] 조회수 증가가 클라이언트에서 직접 처리되지 않고 서버 API 호출로 처리된다.
