...
- [x] `useSpringWebSocket` 구현 — STOMP 방식(`WS /ws/user/interview`, 구독: `/topic/interview/{sessionId}`) 면접 세션 생명주기 이벤트(READY, RUNNING, FINISHED) 핸들링 및 RECONNECTING·ERROR 상태 전이 처리 (재연결 횟수 제한, 복구 불가 시 ERROR 전이)
