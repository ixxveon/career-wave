...
**에러 처리 정책**
* `ERROR`는 **안정 상태(stable state)**다. 자동으로 다른 상태로 전이되지 않으며, 유저가 에러 화면에서 재시도 버튼을 클릭할 때 `ANY → READY` 전이가 발생한다.
* `RECONNECTING → ERROR` 전이 시 에러 화면을 노출하고 재시도 버튼을 제공한다. 자동 `READY` 복원 금지.
...
| TTS 재생 방식 | `TTS_AUDIO` 청크 버퍼 누적 → `TTS_AUDIO_END` 수신 시 병합 후 일괄 재생 | 청크 단위 재생 시 발생하는 끊김 현상 방지 |
...
* **세션 최대 시간**: 전체 세션은 30분(`SESSION_LIMIT_SEC = 1800`)을 초과할 수 없다. 경과 시 `endSession`을 1회 호출하여 강제 종료한다 (`sessionEndedRef`로 중복 호출 방지). 정상 플로우(Q10 자동 종료)의 안전망이다.

* **TTS 버퍼링 재생**: TTS 오디오는 `TTS_AUDIO` 청크를 버퍼에 누적한 뒤 `TTS_AUDIO_END` 메시지를 수신했을 때 전체 청크를 병합하여 한 번에 재생한다. 청크 단위 즉시 재생 방식은 음질 끊김을 유발하므로 금지한다.