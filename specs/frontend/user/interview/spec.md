...
- **FR-003**: 음성 답변 5초 단위 Chunk 분할 전송. STT는 `isFinal=true` 청크 수신 시 누적 청크를 일괄 Whisper 변환하여 최종 결과(`STT_FINAL`)를 채팅창에 반영한다.
- **FR-004**: 문항당 최대 2분 30초(150초) 타이머. 만료 시 `MediaRecorder.stop()` 호출, 잔여 버퍼 전송 후 자동 진행.
- **FR-004-1**: 전체 세션 최대 시간 30분(1800초) 안전망. 경과 시 `endSession` 1회 호출 후 리포트 페이지로 이동. 정상 플로우(Q10 자동 종료)에는 영향 없음.
- **FR-005**: LLM 기반 꼬리 질문·압박 질문 동적 생성. 응답 지연·타임아웃 시 사전 정의 질문으로 폴백.
- **FR-006**: TTS 오디오 버퍼링 후 일괄 재생 — `TTS_AUDIO` 청크를 수신하는 동안 버퍼에 누적하고, `TTS_AUDIO_END` 수신 시 전체 청크를 합쳐 한 번에 재생한다.
...
- **비정상 종료 후 재진입**: `sessionStorage`에 `sessionId` 저장. 재진입 시 `GET /report`로 상태 복원. `IN_PROGRESS` 상태면 세션 복구 안내 노출. 단, 새 세션을 시작하면 서버가 기존 `IN_PROGRESS` 세션을 자동으로 `FAILED` 처리한다.
...
- 면접 최대 질문 개수는 10개로 확정. FastAPI에서 10번째 답변 수신 후 자동으로 리포트 생성을 트리거한다. 프론트엔드는 `REPORT_READY` 수신 시 리포트 페이지로 이동한다.
