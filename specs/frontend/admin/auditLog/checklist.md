...
- [x] logType 값은 `ADMIN_ACTIVITY`, `AI_METRICS_SYSTEM`, `SCRAPING_SYSTEM`으로 통일되어 있다.
- [x] severity 값은 `INFO`, `WARN`, `ERROR`로 통일되어 있다.
...
- [x] `AuditLogType`, `AuditLogSeverity` 타입 정의
...
- [x] logType 탭 조건을 API 조회 조건과 동기화
- [x] severity 필터 조건을 API 조회 조건과 동기화
- [x] keyword 검색 조건을 API 조회 조건과 동기화
- [x] 스크롤 목록 UI에 맞춰 API page/size 기본 조회 조건이 연결된다.
...
- [ ] logType, severity, keyword 변경과 기본 page/size 조회 조건이 API 조회 조건과 일치하는지 확인
- [ ] 로그 선택 시 상세 패널 갱신 확인
