...
3. **Given** 유효한 입력값을 제출했을 때, **When** 생성 API가 성공하면, **Then** 관리자 목록을 재조회하고 감사 로그에 계정 생성 이벤트를 남긴다.
...
2. **Given** ACL 라벨과 CIDR을 입력했을 때, **When** 등록 API가 성공하면, **Then** ACL 목록을 재조회하고 감사 로그에 등록 이벤트를 남긴다.
...
- **FR-009**: 모든 변경 작업은 성공 후 감사 로그가 남아야 한다.
- **FR-010**: 화면은 로딩, 빈 데이터, 검증 실패, 권한 실패, 부분 실패, 전체 실패 상태를 구분해 표시해야 한다.
- **FR-011**: 프론트엔드는 인증 관리자 정보와 세부 역할을 서버 응답 또는 공통 인증 상태 기준으로 판단해야 한다.
