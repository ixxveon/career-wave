...
관리자 AI 매트릭스는 CareerWave의 AI 기능 사용량을 도메인별로 모니터링하기 위한 관리자 화면이다. MVP의 중심은 실제 모델 비용 정산이나 모델 관리가 아니라, 어떤 AI 도메인에서 요청, 토큰, 비용 추정치, 오류가 증가하는지 운영자가 빠르게 파악하는 것이다.
...
현재 화면에는 실제 모델명이 표시될 수 있지만, 추후 DB에서는 화면 표시용 모델명과 실제 호출 모델명을 분리한다. 따라서 프론트엔드와 API 스펙은 아래 필드를 구분한다.
...
팀 기본 규칙은 최신 `develop`에서 기능 브랜치를 생성하는 것이다. 다만 AI 매트릭스 구현은 API 계약, 도메인 사용량, 비용/알림, 로그/이상 사용량이 순차 의존성을 가지므로 팀 합의가 있는 경우 Phase별 스택 브랜치로 진행한다.
