...
- [x] KPI 카드는 mock 데이터가 아니라 API 응답 기반으로 렌더링된다.
- [x] 주요 알림 목록은 API 응답 기반으로 렌더링된다.
- [x] 주간 가입자 차트는 API 응답 기반으로 렌더링된다.
- [x] 결제 비중 차트는 API 응답 기반으로 렌더링된다.
- [x] 관리자 기능 카드는 값과 이동 경로가 API 응답 또는 라우트 상수와 연결되어 있다.
- [x] 시스템 상태는 API 응답 기반으로 렌더링된다.
- [x] 최근 관리자 활동은 API 응답 기반으로 렌더링된다.
- [x] 서버 상태 조회에는 TanStack Query가 우선 적용되어 있거나, 미적용 사유가 남아 있다.
...
- [x] 일부 섹션 조회 실패 시 전체 화면이 막히지 않고 해당 섹션만 실패 상태가 표시된다.
...
- [ ] 주요 알림 버튼은 올바른 관리자 페이지로 이동한다.
- [ ] 기능 카드 상세 보기 버튼은 올바른 관리자 페이지로 이동한다.
