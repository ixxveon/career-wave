...
- [ ] 파이프라인 상태는 `ACTIVE`, `WARNING`, `FAILED`, `RECOVERING`, `PAUSED`로 통일되어 있다.
- [ ] 로그 등급은 `INFO`, `WARN`, `ERROR`, `SUCCESS`로 통일되어 있다.
...
- [ ] `ScrapingPipelineStatus`, `ScrapingLogLevel`, `ScrapingActionType` 타입이 정의되어 있다.
- [ ] `ScrapingPipeline`, `ScrapingPipelineSummary`, `ScrapingActionRequest`, `ScrapingActionResult`, `ScrapingLog` 응답 타입이 정의되어 있다.
- [ ] `ApiResponse<T>` 응답 구조를 기준으로 처리한다.
- [ ] TypeScript interface는 PascalCase를 사용하고 `I` prefix를 사용하지 않는다.

## Phase 2 - 파이프라인 목록, 검색, 필터

- [ ] 파이프라인 목록이 API 데이터로 렌더링된다.
- [ ] source명 또는 최근 오류 검색 조건이 API 조회 조건과 동기화된다.
- [ ] 상태 필터가 API 조회 조건과 동기화된다.
...
- [ ] 로그 등급 `INFO`, `WARN`, `ERROR`, `SUCCESS`가 구분되어 표시된다.
- [ ] 파이프라인 또는 로그 등급 필터가 API 조회 조건과 동기화된다.
- [ ] 로그 페이지네이션이 API 응답 메타데이터와 일치한다.
- [ ] 실패 파이프라인의 최근 오류와 로그 상세가 연결되어 보인다.
...
- [ ] `PAUSED`, `FAILED`, `RECOVERING` 상태별 버튼 활성화 정책이 확인된다.
