...
- [x] 실행 액션이 API와 연결되어 있다.
- [x] 재시도 액션이 API와 연결되어 있다.
- [x] 테스트 액션이 API와 연결되어 있다.
- [x] 액션 요청 중 중복 클릭이 방지된다.
- [x] 액션 성공 후 목록 데이터가 갱신된다.
- [x] 액션 실패 시 오류 안내가 표시된다.

## Phase 4 - 운영 로그와 장애 원인 확인

- [x] 운영 로그 콘솔이 API 데이터로 표시된다.
- [x] 로그 상태 `SUCCESS`, `FAILED`가 구분되어 표시된다.
- [x] source 또는 로그 상태 필터가 API 조회 조건과 동기화된다.
- [x] 로그 페이지네이션이 API 응답 메타데이터와 일치한다.
- [x] 실패 source의 최근 오류와 로그 상세가 연결되어 보인다.
- [x] 로그 상세에 토큰, 쿠키, 프록시 주소, 외부 응답 전문이 노출되지 않는다.

## Phase 5 - 검증 및 마감

- [ ] 관리자 JWT와 `ROLE_ADMIN` 권한 기준 접근 제어가 확인된다.
- [ ] `SecurityConfig` 또는 동등한 보안 설정에서 `/api/v1/admin/scraping/**`가 `hasRole("ADMIN")`, `hasAuthority("ROLE_ADMIN")` 또는 프로젝트 표준 방식으로 보호된다.
