...
- [ ] `ScrapingPipelineStatus`, `ScrapingLogLevel`, `ScrapingActionType` 타입 정의
- [ ] `ScrapingPipeline`, `ScrapingPipelineSummary`, `ScrapingActionRequest`, `ScrapingActionResult`, `ScrapingLog` 응답 타입 정의
...
- [ ] 파이프라인 목록을 API 데이터로 렌더링
- [ ] source명 또는 최근 오류 검색 조건 연결
- [ ] 상태 필터 `ACTIVE`, `WARNING`, `FAILED`, `RECOVERING`, `PAUSED` 연결
...
- [ ] 로그 등급 `INFO`, `WARN`, `ERROR`, `SUCCESS` 표시
- [ ] 파이프라인 또는 로그 등급 필터 연결
- [ ] 로그 페이지네이션 연결
- [ ] 실패 파이프라인의 최근 오류와 로그 상세 연결
...
- [ ] `PAUSED`, `FAILED`, `RECOVERING` 상태별 버튼 활성화 정책 확인
