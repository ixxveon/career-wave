...
- 409 (`REFUND_NOT_PENDING`) 응답은 별도 에러 메시지로 처리한다.
...
- 결제·환불 상태 배지는 서버 응답 기준으로만 갱신한다.
- 환불 처리 확정·불가 처리 성공 후 목록을 재조회하거나 서버 응답으로 해당 행만 업데이트한다.
- 낙관적 업데이트(Optimistic Update)를 사용하지 않는다.

### 로딩·에러 처리

- API 호출 중에는 테이블 영역에 로딩 표시를 한다.
- 환불 처리 모달 진행 중에는 버튼을 `disabled` 처리한다.
