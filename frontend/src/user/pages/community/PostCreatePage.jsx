        ...
        <div className="pc-card">
          <div className="pc-field">
            ...
            <div className="pc-cats">
              {CATEGORIES.map(c => (
                <button key={c} className={`pc-cat${category === c ? ' pc-cat--on' : ''}`} onClick={() => setCategory(c)}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div className="pc-field">
            ...
            <input className="pc-input" placeholder="제목을 입력하세요" maxLength={100} value={title} onChange={e => setTitle(e.target.value)} />
            <span className="pc-field__count">{title.length} / 100</span>
          </div>

          <div className="pc-field">
            <label className="pc-field__label">내용 <span>*</span></label>
            <textarea className="pc-textarea" placeholder="내용을 작성하세요. 경험과 노하우를 구체적으로 공유해 주세요." rows={14} value={content} onChange={e => setContent(e.target.value)} />
          </div>
