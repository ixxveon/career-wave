...
export const memberRecoveryApi = {
  findId(payload: FindIdRequest): Promise<FindIdResponse> {
    return memberApiClient<FindIdResponse>('/api/v1/members/recovery/find-id', {
      method: 'POST',
  ...
  issuePasswordToken(payload: PasswordTokenRequest): Promise<PasswordTokenResponse> {
    return memberApiClient<PasswordTokenResponse>('/api/v1/members/recovery/password-token', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  },
  ...
  resetPassword(payload: ResetPasswordRequest): Promise<ResetPasswordResponse> {
    return memberApiClient<ResetPasswordResponse>('/api/v1/members/recovery/reset-password', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  },
