...
const severityOptions: Array<{ value: AuditLogSeverityFilter; label: string }> = [
  { value: AUDIT_LOG_SEVERITY_FILTER.ALL, label: '전체 상태' },
...
const AUDIT_LOG_LIST_PAGE_SIZE = 9;
...
export default function AuditLogPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [dateRangePreset, logTypeFilter, severityFilter, adminId, debouncedKeyword]);
  ...
  const {
    ...
    queryKey: ['admin', 'auditLog', 'list', dateRangePreset, logTypeFilter, severityFilter, adminId, debouncedKeyword, page],
    queryFn: async () => {
      const response = await auditLogApi.getLogs({
        ...dateRange,
        page,
        size: AUDIT_LOG_LIST_PAGE_SIZE,
  ...
  const placeholderRowCount = !isListLoading && !isListError && auditLogs.length > 0
    ? Math.max(0, AUDIT_LOG_LIST_PAGE_SIZE - auditLogs.length)
    : 0;
    ...
    <section className="auditOpsPage">
      ...
      <section className="auditOpsShell">
        <div className="auditOpsFilters" aria-label="감사 로그 필터">
          ...
          <select value={severityFilter} onChange={(event) => setSeverityFilter(event.target.value as AuditLogSeverityFilter)} aria-label="상태">
        ...
        <div className="auditOpsListHeader">
          ...
          <span>페이지당 {AUDIT_LOG_LIST_PAGE_SIZE}건</span>
        </div>
        ...
        <div className="auditOpsGrid">
          <div className="auditOpsTableWrap">
            ...
            <div className="auditOpsTableBody">
              ...
              {Array.from({ length: placeholderRowCount }, (_, index) => (
                <div key={`audit-placeholder-${index}`} className="auditOpsTableRow placeholder" aria-hidden="true" />
              ))}
          ...
          <aside className={`auditOpsDetail ${isMobileDetailOpen ? 'mobile-open' : ''}`} aria-label="선택 로그 상세">
            ...
            {selectedDisplay ? <div className="auditOpsDetailBody">
              <div className="auditOpsDetailAction"><strong>{selectedDisplay.summary}</strong><span className={`auditOpsTag ${severityToneMap[selectedDisplay.severity]}`}>{selectedDisplay.severity}</span></div>
              <div className={`auditOpsDetailNoticeSlot ${!isDetailLoading && !isDetailError ? 'placeholder' : ''}`} aria-hidden={!isDetailLoading && !isDetailError}>
        ...
        <nav className={`auditOpsPagination ${totalPages > 0 ? '' : 'placeholder'}`} aria-label="감사 로그 페이지 이동" aria-hidden={totalPages === 0}><button type="button" disabled={page === 1 || totalPages === 0} onClick={() => setPage(page - 1)}>이전</button>{pageNumbers[0] > 1 ? <><button type="button" onClick={() => setPage(1)}>1</button><span>...</span></> : null}{pageNumbers.map((pageNumber) => <button key={pageNumber} type="button" className={pageNumber === page ? 'active' : ''} onClick={() => setPage(pageNumber)}>{pageNumber}</button>)}{pageNumbers[pageNumbers.length - 1] < totalPages ? <><span>...</span><button type="button" onClick={() => setPage(totalPages)}>{totalPages}</button></> : null}<button type="button" disabled={page === totalPages || totalPages === 0} onClick={() => setPage(page + 1)}>다음</button></nav>
      </section>
    </section>
  );
}
