...
public interface AiMetricsDocs {
    ...
    ResponseEntity<kr.co.carrer.global.response.ApiResponse<AiMetricsDTO.ResponseSummary>> getSummary(
            ...
            @Parameter(description = "AI 기능 유형 필터, ai_usage_logs.feature_type 기준", schema = @Schema(implementation = AiFeatureType.class)) @RequestParam(required = false) AiFeatureType featureType
    );
